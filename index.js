	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `iae @${num.split('@')[0]}\nbem vindo ao grupo *${mdata.subject}* le as regras ai pra evitar ban`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remover') {
				num = anu.participants[0]
				try {
				...
				teks = `ESSE E DEPRESSIVO🐊🧐🙌 @${num.split('@')[0]}`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				success: '✔️ pronto seu sticker esta feito ✔️',
				error: {
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'voce tem tudo isso de contato bloqueado :\n'
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falha, na época converter ${tipe} o stiker`)
							})
							.on('end', function () {
								console.log('Finish')
								buff = fs.readFileSync(ran)
				...
				case 'gtts':
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('O texto e mto grande pfvr diminua')
				...
				case 'add':
					...
					if (args.length < 1) return reply('A que você deseja adicionar um gênio?kkkkkkkkkkkk')
					if (args[0].startsWith('08')) return reply('Use o código do país, exemplo:(55) (1) etc...')
					try {
				...
				case 'ftostiker':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'cheira meu canekin'})
						fs.unlinkSync(ran)
					})
					break
				case 'simi':
				...
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('SEI KKKKKKKK TA ATIVADO')
					if (Number(args[0]) === 1) {
						...
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Sukes desative o modo simi neste grupo ✔️')
					} else {
				...
				case 'bemvindo':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('BEM VINDO ATIVADO')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('ATIVADO ADM GOXTOSO😳👉👈')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Sukses ative o recurso de boas-vindas neste grupo ✔️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Sukses desative o recurso de boas-vindas neste grupo ✔️')
					} else {
						reply('1 para ativar, 0 para desativar')
					}
				case 'clone':
